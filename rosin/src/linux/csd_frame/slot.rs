...
use crate::linux::csd_frame::raw::CreatePoolError;
use crate::linux::csd_frame::raw::RawPool;
use std::io;
use std::{
...
impl SlotPool {
    pub fn new(len: usize, shm: &wl_shm::WlShm) -> Result<Self, CreatePoolError> {
        let inner = RawPool::new(shm, len)?;
    ...
    pub fn create_buffer(&mut self, width: i32, height: i32, stride: i32, format: wl_shm::Format) -> Result<(Buffer, &mut [u8]), CreateBufferError> {
        let len = (height as usize) * (stride as usize);
        let slot = self.new_slot(len).expect("msg");
    ...
    ///   advertised to the `wl_shm` global by the server is a protocol error and will
    ///   terminate your connection
    pub fn create_buffer_in(&mut self, slot: &Slot, width: i32, height: i32, stride: i32, format: wl_shm::Format) -> Result<Buffer, CreateBufferError> {
...
impl Drop for Slot {
    fn drop(&mut self) {
        if self.inner.all_refs.fetch_sub(1, Ordering::Relaxed) == 1
            && let Some(free_list) = self.inner.free_list.upgrade()
        {
...
impl wayland_client::backend::ObjectData for BufferData {
    fn event(
        ...
        debug_assert!(wayland_client::backend::protocol::same_interface(msg.sender_id.interface(), wl_buffer::WlBuffer::interface()));
        ...
        match self.state.fetch_or(BufferData::RELEASE_SET, Ordering::Relaxed) {
            ...
            BufferData::DESTROY_ON_RELEASE => {
                ...
                handle.send_request(msg.map_fd(|x| x.as_raw_fd()), None, None).expect("Unexpected invalid ID");
            }
            BufferData::DEAD => {
