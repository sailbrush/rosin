...
pub(crate) fn create_window_x11<S: Any + Sync + 'static, T: x11rb::connection::Connection>(
    ...
    atoms: &super::x11_state::AtomCollection,
    depth: u8, visual_id: Visualid
    ...
    let win_aux = CreateWindowAux::new()
        ...
        .colormap(colormap);
    x11rb::protocol::xproto::ConnectionExt::create_window(&conn, depth, window, screen.root, 0, 0, desc.size.width as u16, desc.size.height as u16, 0, WindowClass::INPUT_OUTPUT, visual_id, &win_aux)?;
    ...
    conn.change_property32(
        ...
        atoms.WM_PROTOCOLS,
        AtomEnum::ATOM,
        &[atoms.WM_DELETE_WINDOW].as_slice(),
    )?;
    conn.change_property8(
        PropMode::REPLACE,
        window,
...
pub(crate) fn create_window_wayland<S: Any + Sync + 'static>(
    desc: &WindowDesc<S>, globals: &GlobalList, qh: &QueueHandle<RosinWaylandWindow<S>>
) -> Window {
